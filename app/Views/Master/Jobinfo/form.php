<!-- Modal body -->
<form id="form-jobinfo" method="post" class="p-2 md:p-3">
    <input type="hidden" name="id" value="<?= $formType == 'edit' ? $id : '' ?>">
    <div class="grid gap-2 mb-4 grid-cols-2">
        <div class="col-span-2">
            <label for="name" class="block mb-0.5 text-xs text-gray-800 after:content-['*'] after:text-red-600 after:ml-0.5">Job Information</label>
            <input value="<?= $formType == 'edit' ? $res->info : '' ?>" type="text" name="info" id="info" class="bg-gray-50 border border-gray-300 text-gray-900 text-xs rounded-[4px] focus:!ring-gozi-400 block w-full p-2.5" placeholder="example: BKI SKARIGA" required="">
        </div>
    </div>
...
<script>
    $(document).ready(function() {
        $("#form-jobinfo").on("submit", function(e) {
            e.preventDefault();
            let formType = "<?= $formType ?>";
            let link = "<?= site_url('jobinfo/add') ?>";
            if (formType == "edit") {
                link = "<?= site_url('jobinfo/update') ?>";
            }
