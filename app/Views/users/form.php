<!-- Modal body -->
<form id="form-users" method="post" class="p-2 md:p-3">
    ...
    <div class="grid gap-2 mb-4 grid-cols-2">
        <div class="col-span-2">
            ...
            <input value="<?= $formType == 'edit' ? $res->username : '' ?>" type="text" name="username" id="name" class="bg-gray-50 border border-gray-300 text-gray-900 text-xs rounded-[4px] focus:!ring-gozi-400 block w-full p-2.5" placeholder="example: 22519/1990.063" required="">
        </div>
        <div class="col-span-2">
            <label for="category" class="block mb-0.5 text-xs text-gray-800 after:content-['*'] after:text-red-600 after:ml-0.5">Role</label>
            <select id="category" name="role" class="bg-gray-50 border border-gray-300 text-gray-900 text-xs rounded-[4px] block w-full p-2.5 focus:!ring-gozi-400">
                <option value="user" <?= (isset($res->role) && $res->role === 'user') ? 'selected' : '' ?>>User</option>
                <option value="admin" <?= (isset($res->role) && $res->role === 'admin') ? 'selected' : '' ?>>Admin</option>
            </select>
        </div>
        <div class="col-span-2">
            <label for="name" class="block mb-0.5 text-xs text-gray-800 after:content-['*'] after:text-red-600 after:ml-0.5">Password</label>
            <input value="<?= $formType == 'edit' ? (empty($res->password) ? '-' : $res->password) : '' ?>" type="text" name="password" id="password" class="bg-gray-50 border border-gray-300 text-gray-900 text-xs rounded-[4px] focus:!ring-gozi-400 block w-full p-2.5" placeholder="example: $2a$12$ar.A3NtxJibqJuH24vEvT" required="">
        </div>
    </div>
...
<script>
    $(document).ready(function() {
        $("#form-users").on("submit", function(e) {
            ...
            let link = "<?= site_url('users/add') ?>";
            if (formType == "edit") {
                link = "<?= site_url('users/update') ?>";
            }
